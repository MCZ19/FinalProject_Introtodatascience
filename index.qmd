...
extract_cluster_assignment() |>  
  bind_cols(schools_cond, cluster = educ_kmeans_1 |> pull(.cluster)) |>
  group_by(cluster) |>
  summarize(
    mean(ela_proficient),
    mean(math_4),
...
educ_rec_pca %>% prep() %>%
  tidy(id = "pca", type = "variance") %>% filter(terms == "variance") %>% mutate(pov_pct_var = poverty_status / sum(poverty_status)) %>% slice_head(n = 2)
```
