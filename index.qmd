...
block_stats_clean <- block_stats %>%
  ...
  mutate(medicaid = (medicaid_19under +
                       ...
                       medicaid_65older)) #|>
  #aggregate()
#^^^ Need to combine by GEOID, and sum the values associated with each GEOID

demographics <- merge(demographics, block_stats_clean[, c("GEOID", "population")], by = "GEOID", all.x = TRUE)
